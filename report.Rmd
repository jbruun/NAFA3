...
wordstoremove<-c(stopwords("english"),"can","also","although","may","maybe","moreover",
                 ...
                 "france", "highly","hitherto","important","importance","mentions","might","much","must","namely","necessary","next",
                 "often","one","ones","put","several","similar","since","socalled","summary","three", "two", "typical","us","usually","view","virtue","waddington",
                 "well","whereas","whether","whose","within")
...
for (i in 1:n){
  sol<-read.csv(paste0("~/Github/NAFA2/infomapSolutions/collected/sol",i,".clu"),skip=8,sep=" ")
...
for (i in 1:n){
  sol<-read.csv(paste0("~/Github/NAFA2/infomapSolutions/bildung/sol",i,".clu"),skip=8,sep=" ")
  names(sol)<-c("node","membership","flow","NA")
  sol<-sol[,-4]
  sol <- sol[order(sol$node),]
...
for (i in 1:n){
  sol<-read.csv(paste0("~/Github/NAFA2/infomapSolutions/competence/sol",i,".clu"),skip=8,sep=" ")
  names(sol)<-c("node","membership","flow","NA")
  sol<-sol[,-4]
  sol <- sol[order(sol$node),]
...
#REPEAT FOR COMPETENCE AND BILDUNG!!
which.max(no)
which.max(noC)
which.max(noB)
```
...
```{r fast-greedy, echo=TRUE}


memBB<-read.csv(paste0("~/Github/NAFA2/infomapSolutions/collected/sol",which.max(no),".clu"),skip=8,sep=" ")
names(memBB)<-c("node","membership","flow","NA")
memBB<-memBB[,-4]
memBB <- memBB[order(memBB$node),]

#find a membership solution for competence networks
memCBB<-read.csv(paste0("~/Github/NAFA2/infomapSolutions/competence/sol",which.max(noC),".clu"),skip=8,sep=" ")
names(memCBB)<-c("node","membership","flow","NA")
memCBB<-memCBB[,-4]
memCBB <- memBB[order(memCBB$node),]

#find a membership solution for competence networks
memBBB<-read.csv(paste0("~/Github/NAFA2/infomapSolutions/bildung/sol",which.max(noB),".clu"),skip=8,sep=" ")
...
```{r loads, echo=FALSE}
#Find out how much each separate text network loads on each module
...
write.graph(collectedNetwork, paste0("networks/collectedNetwork", format(Sys.time(), "%d-%b-%Y-%H-%M"), ".net"),format="pajek")

write.graph(collectedNetwork, paste0("networks/collectedNetwork", format(Sys.time(), "%d-%b-%Y-%H-%M"), ".graphml"),format="graphml")

write.graph(collectedBildungNetworkBB, paste0("networks/collectedBildungNetworkBB", format(Sys.time(), "%d-%b-%Y-%H-%M"), ".net"),format="pajek")
...
write.graph(collectedCompetenceNetworkBB, paste0("networks/collectedCompetenceNetworkBB", format(Sys.time(), "%d-%b-%Y-%H-%M"), ".net"),format="pajek")
...
write.graph(collectedNetworkBB, paste0("networks/collectedNetworkBB", format(Sys.time(), "%d-%b-%Y-%H-%M"), ".net"),format="pajek")

write.graph(collectedNetworkBB, paste0("networks/collectedNetworkBB", format(Sys.time(), "%d-%b-%Y-%H-%M"), ".graphml"),format="graphml")

write.graph(collectedMapBB, paste0("networks/collectedMapBB", format(Sys.time(), "%d-%b-%Y-%H-%M"), ".net"),format="pajek")
